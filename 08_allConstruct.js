...
// []
console.log(allConstruct('aaaaaaaaaaaaaaaaaz',['a','aa','aaa','aaaa','aaaaaa']));
// []

const allConstruct_optim= (target, wordBank,memo={}) => {
    ...
    for (let word of wordBank) {
        if (target.indexOf(word) === 0){
            const suffix = target.slice(word.length);
            const suffixWays = allConstruct_optim(suffix, wordBank,memo);
            const targetWays = suffixWays.map(way => [word, ...way]);
            result.push(...targetWays);
        }
    }
...
console.log(allConstruct_optim('purple',['purp','p','ur','le','purpl']));
//[['purp','le'],['p','ur','p','le']]
console.log(allConstruct_optim('abcdef',['ab','abc','cd','def','abcd','ef','c']));
//[['ab','cd','ef'],['ab','c','def'],['abc','def'],['abcd','ef]]
console.log(allConstruct_optim('skateboard',['bo','rd','ate','t','ska','sk','boar']));
// []
console.log(allConstruct_optim('aaaaaaaaaaaaaaaaaz',['a','aa','aaa','aaaa','aaaaaa']));
// []